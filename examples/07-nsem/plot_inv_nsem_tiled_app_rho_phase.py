...
def run():
    ...
    for rx_orientation in ['xy', 'yx']:
        rxList.append(ns.Rx.Point3DComplexResistivity(locations=None, locations_e=receiver_locations, locations_h=receiver_locations, orientation=rx_orientation, component='apparent_resistivity'))
        rxList.append(ns.Rx.Point3DComplexResistivity(locations=None,locations_e=receiver_locations, locations_h=receiver_locations, orientation=rx_orientation, component='phase'))
    ...
    # make data object
    global_data = simulation.make_synthetic_data(model, relative_error=0.1, noise_floor=5e-3, add_noise=True)
    ...
    for freq in frequencies:
        ...
        for rx_orientation in range(len(comps_)):
            pert = np.percentile(np.abs(dnew[cnt, num_station*rx_orientation:num_station*(rx_orientation + 1)]), 20)
