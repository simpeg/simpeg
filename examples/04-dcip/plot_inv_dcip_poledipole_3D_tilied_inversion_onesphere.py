...
# print(stns_x_tx.min(), stns_x_tx.max())
# now do rx lines
num_lines = int(num_lines)
spacing = float(line_spacing)
...
for line in range(stns_x_tx.shape[1]):
    ...
    # print('after: ', stns_x_rx.shape, stns_y_rx.shape)
    # stns_x_tx = stns_x_tx.flatten('F')
    # stns_y_tx = stns_y_tx.flatten('F')

    tx = transmit()
    # print('doin: ', np.vstack((stns_x_tx[:, line], stns_y_tx[:, line])).shape, np.vstack((stns_x_rx, stns_y_rx)).shape)
...
survey = dc.Survey(src_lists)          # creates the survey
# check if data is IP
if data_type == "IP":
...
survey.drapeTopo(global_mesh, active_cells, option='top') 
...
simulation = dc.Simulation3DNodal(
    global_mesh, survey=survey, sigmaMap=mapping, solver=Solver
)

global_data = simulation.make_synthetic_data(mtrue[active_cells], relative_error=0.05, add_noise=True)
...
@dask.delayed
def create_tile_dc(source, obs, uncert, global_mesh, global_active, tile_id, workers=None):
...
survey.std = np.abs(global_data.dobs * 0.05)
print("[INFO] Creating tiled simulations over sources")
eps = np.percentile(np.abs(survey.dobs), 10)
survey.std = np.abs(survey.dobs * 0.05) + eps
start_ = time.time()
local_misfits = []
idx_start = 0
idx_end = 0
...
for ii, source in enumerate(survey.source_list):
    if cnt == 29:
        ...
        delayed_misfit = create_tile_dc(
                    ...
                    survey.std[idx_start:idx_end], global_mesh, active_cells, ii, workers=None)
