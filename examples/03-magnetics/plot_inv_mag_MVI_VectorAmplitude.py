...
reg_direction = reg_p + reg_s + reg_t
...
reg_ampplitude = reg_x + reg_y + reg_z

reg = reg_direction# + reg_ampplitude# +
...
update_ref = directives.UpdateReferenceVector(
    reg_direction, mkvc(utils.mat_utils.dip_azimuth2cartesian(model_azm_dip[:, 0], model_azm_dip[:, 1]))
)

update_dir = directives.UpdateReferenceVector(
    reg_ampplitude, mkvc(model), component="amplitude"
)
