...
opt = Optimization.InexactGaussNewton(maxIter=10,  tolX=1e-6)
opt.remember('xc')
...
inv = Inversion.BaseInversion(invProb, directiveList=[beta, betaSched, petrodir, smoothref])
...
clf.means_ += np.random.randn(3, 1)
...
gamma_petro = np.ones(clf.n_components)*1.
petrodir = Directives.GaussianMixtureUpdateModel(
      alphadir=gamma_petro, nu=gamma_petro, kappa=100.*gamma_petro
)
invProb.reg.gamma = gamma_petro
target = Directives.TargetMisfit()
...
inv = Inversion.BaseInversion(invProb, directiveList=[beta, target, betaSched, petrodir, smoothref])
...
ax[3].set_title('Petrophysically constrained: No Mean Prior')
