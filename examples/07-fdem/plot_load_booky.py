...
def run(plotIt=True):
    ...
    resolve = np.loadtxt(directory+"Bookpurnong_Resolve_Exported.XYZ", skiprows=8)

    # Omit few cross lines
    resolve = resolve[np.logical_and(resolve[:, -1]>30002, resolve[:,-1]<38000), :]
    ...
    # Load SkyTEM (2006)
    fid = open(directory+"SK655CS_Bookpurnong_ZX_HM_TxInc_newDTM.txt", "rb")
    lines = fid.readlines()
    fid.close()
    ...
    river_path = np.loadtxt(directory+"MurrayRiver.txt")
    ...
    dobs_re = np.load(directory+"dobs_re_final.npy")
    dpred_re = np.load(directory+"dpred_re_final.npy")
    mopt_re = np.load(directory+"mopt_re_final.npy")

    # Down sample resolve data
    nskip = 40
    inds_resolve = np.r_[np.array(range(0, data_resolve.shape[0]-1, nskip)), 16730]

    booky_resolve = {"data": data_resolve[inds_resolve, :],
    ...
    area = 314.
    waveform = np.loadtxt(directory+"skytem_hm.wf")
    times = np.loadtxt(directory+"skytem_hm.tc")

    booky_skytem = {"data": data_skytem,
    ...
    save_dict_to_hdf5("booky_resolve.hdf5", booky_resolve)
    save_dict_to_hdf5("booky_skytem.hdf5", booky_skytem)
