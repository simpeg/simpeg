...
n_loc = 20  # Number of electrode locations along EW profile
xr = np.linspace(-300., 300., n_loc)
zr = np.zeros(n_loc)
electrode_locs = np.c_[mkvc(xr), mkvc(zr)]
...
while ii < n_loc:
    ...
    rx_list = [dc.receivers.Dipole_ky(m_locs, n_locs)]
...
simulation_dc = dc.simulation_2d.Problem2D_N(mesh, survey=survey_dc, sigmaMap=mod_dc_map, Solver=Solver)
...
ax1 = fig.add_axes([0.05, 0.05, 0.8, 0.9])
plot_pseudoSection(
    survey_dc, ax=ax1, survey_type='dipole-dipole',
