...
background_conductivity_value = np.log(100.0)
sphere_conductivity_value = np.log(70.0)
dyke_conductivity_value = np.log(40.0)
...
model = np.kron(
    np.ones((N, 1)), np.c_[background_conductivity_value, background_mu_value]
)
...
ind_sphere = ind_sphere[ind_active]  # So same size and order as model
model[ind_sphere, :] = np.c_[sphere_conductivity_value, sphere_mu_value]
...
ind_polygon = model_builder.get_indices_polygon(mesh, xyz_pts)
ind_polygon = ind_polygon[ind_active]  # So same size and order as model
model[ind_polygon, 0] = dyke_conductivity_value
