"""
Forward Simulation of Stitched Frequency-Domain Data with Height
================================================================
...
source_heights = 30.*np.ones(n_sounding)
source_locations = np.c_[x, np.zeros(n_sounding), 30 *np.ones(n_sounding)]
source_current = 1.
source_radius = 5.
moment_amplitude = 1.

source_receiver_offsets = np.c_[10.*np.ones(n_sounding), np.zeros(n_sounding), np.zeros(n_sounding)]
...
for ii in range(0, n_sounding):
    ...
    receiver_location = mkvc(source_receiver_offsets[ii, :])
...
wires = maps.Wires(('sigma', n_param),('h', n_sounding))
sigma_map = maps.ExpMap(nP=n_param) * wires.sigma
h_map = wires.h

model = np.r_[sounding_models, source_heights]
...
simulation = em1d.simulation.StitchedEM1DFMSimulation(
    survey=survey, thicknesses=thicknesses, sigmaMap=sigma_map, hMap=h_map, topo=topo,
