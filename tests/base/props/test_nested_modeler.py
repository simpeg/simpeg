...
class NotInvertibleInnerProp(InnerProp):
    phys_prop = InnerProp.phys_prop.set_feature(invertible=False)


class NestedSim(props.HasModel):
...
def test_has_nested():
    assert NestedSim._has_nested_models is True


def test_unset_nested():
    pm = NestedSim()
    with pytest.raises(
        AttributeError, match=r"NestedModeler `nest_model` has not been set\."
    ):
        pm.nest_model
