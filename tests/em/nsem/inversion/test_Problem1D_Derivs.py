...
def DerivJvecTest(halfspace_value, freq=False, expMap=True):
    ...
    problem = NSEM.Simulation1D_ePrimSec(
        ...
        sigmaPrimary=sigBG, sigmaMap=simpeg.Maps.IdentityMap(mesh))
...
def DerivProjfieldsTest(inputSetup,comp='All',freq=False):
    ...
    np.random.seed(1983)
    u0x = (np.random.randn(survey.mesh.nE) +
        np.random.randn(survey.mesh.nE) * 1j
    )
    u0y = (np.random.randn(survey.mesh.nE) +
        np.random.randn(survey.mesh.nE) * 1j
    )
    u0 = np.vstack((simpeg.mkvc(u0x, 2), simpeg.mkvc(u0y, 2)))
    f0 = problem.fieldsPair(mesh=survey.mesh, survey=survey)
    ...
    def fun(u):
        f = problem.fieldsPair(mesh=survey.mesh, survey=survey)
