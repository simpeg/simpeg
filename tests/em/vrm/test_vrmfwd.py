...
class VRM_fwd_tests(unittest.TestCase):
    ...
    def test_predict_dipolar(self):
        ...
        self.assertTrue(
            np.all(
                np.abs(Fields[1:-1:3] - np.r_[fx, fy, fz])
                < 1e-5 * np.sqrt((Fields[1:-1:3] ** 2).sum())
            )
        )
    ...
    def test_sources(self):
        ...
        err1 = np.all(
            np.abs(Fields[9:18] - Fields[0:9]) / (np.abs(Fields[0:9]) + 1e-14) < 0.005
        )
        err2 = np.all(
            np.abs(Fields[18:] - Fields[0:9]) / (np.abs(Fields[0:9]) + 1e-14) < 0.005
        )
        err3 = np.all(
            np.abs(Fields[9:18] - Fields[18:]) / (np.abs(Fields[18:]) + 1e-14) < 0.005
        )
    ...
    def test_convergence_vertical(self):
        ...
        Errs = np.abs(
            (np.r_[Fields2, Fields3, Fields4, Fields5] - Fields_true) / Fields_true
        )
    ...
    def test_convergence_radial(self):
        ...
        ErrsX = np.abs(
            (np.r_[Fields2[0], Fields3[0], Fields4[0], Fields5[0]] - Fields_true)
            / Fields_true
        )
        ErrsY = np.abs(
            (np.r_[Fields2[1], Fields3[1], Fields4[1], Fields5[1]] - Fields_true)
            / Fields_true
        )
