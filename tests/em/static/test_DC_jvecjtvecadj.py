...
class DCProblemTestsN_Axial_Anisotropy(unittest.TestCase):
    def setUp(self):
        aSpacing = 2.5
        nElecs = 10
        ...
        nP = 2 * mesh.nC
        wires_map = maps.Wires(("sigma_x", mesh.nC), ("sigma_z", mesh.nC))
        simulation = dc.simulation.Simulation3DNodal(
            mesh=mesh, survey=survey, rhoMap=maps.IdentityMap(nP=nP), bc_type="Neumann"
        )
...
class DCProblemTestsCC_Axial_Anisotropy(unittest.TestCase):
    def setUp(self):
        aSpacing = 2.5
        nElecs = 10
