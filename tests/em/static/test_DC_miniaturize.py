...
class DCMini2DTestSurveyTypes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.d_d_survey = gen_DCIPsurvey(
            survey_end_points,
            'dipole-dipole', aSpacing, aSpacing, nElecs, dim=2
        )
        self.d_p_survey = gen_DCIPsurvey(
            survey_end_points,
            'dipole-pole', aSpacing, aSpacing, nElecs, dim=2
        )
        self.p_d_survey = gen_DCIPsurvey(
            survey_end_points,
            'pole-dipole', aSpacing, aSpacing, nElecs, dim=2
        )
        self.p_p_survey = gen_DCIPsurvey(
            survey_end_points,
            'pole-pole', aSpacing, aSpacing, nElecs, dim=2
        )

    def test_dipole_dipole_mini(self):
        sim1 = dc.Simulation2DNodal(
                mesh=self.mesh, survey=self.d_d_survey,
    ...
    def test_dipole_pole_mini(self):
        sim1 = dc.Simulation2DNodal(
                mesh=self.mesh, survey=self.d_p_survey,
                sigmaMap=maps.IdentityMap(self.mesh)
        )
    ...
    def test_pole_dipole_mini(self):
        sim1 = dc.Simulation2DNodal(
                mesh=self.mesh, survey=self.p_d_survey,
                sigmaMap=maps.IdentityMap(self.mesh)
        )
    ...
    def test_pole_pole_mini(self):
        sim1 = dc.Simulation2DNodal(
                mesh=self.mesh, survey=self.p_p_survey,
                sigmaMap=maps.IdentityMap(self.mesh)
        )
...
class DC2DMiniaturizeTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        rx_d = dc.receivers.Dipole(electrode_locations[[2, 4, 6]], electrode_locations[[3, 5, 9]])

        tx_pd = dc.sources.Pole([rx_d], electrode_locations[0])
        tx_pp = dc.sources.Pole([rx_p], electrode_locations[1])
        tx_dp = dc.sources.Dipole([rx_p], electrode_locations[0], electrode_locations[1])
