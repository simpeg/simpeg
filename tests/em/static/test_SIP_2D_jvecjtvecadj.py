...
class SIPProblemTestsN_air(unittest.TestCase):
    def setUp(self):
        ...
        # Now set up the problem to do some minimization
        dmis = data_misfit.L2DataMisfit(data=dobs, simulation=problem)
        reg_eta = regularization.Simple(mesh, mapping=wires.eta, active_cells=~airind)
        reg_taui = regularization.Simple(mesh, mapping=wires.taui, active_cells=~airind)
        reg_c = regularization.Simple(mesh, mapping=wires.c, active_cells=~airind)
