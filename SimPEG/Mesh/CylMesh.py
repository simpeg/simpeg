...
class CylMesh(BaseTensorMesh, BaseRectangularMesh, InnerProducts, CylView):
    ...
    def _fastInnerProduct(self, projType, prop=None, invProp=False,
                          invMat=False):
        """
            Fast version of getFaceInnerProduct.
            This does not handle the case of a full tensor prop.

            :param numpy.array prop: material property (tensor properties are
                                     possible) at each cell center
                                     (nC, (1, 3, or 6))
        ...
        if prop.size == self.nC:
            ...
            if projType == 'E':
                # for cyl mesh, only y-diagonal used in edge inner product
                prop = prop[:, 1]
                V = Utils.sdiag(self.vol)
            elif projType == 'F':
                # for cly mesh, only x, z components used in face inner product
                prop = prop[:, [0, 2]]
    ...
    def _fastInnerProductDeriv(self, projType, prop, invProp=False,
        ...
        if tensorType == 2:  # anisotropic
            ...
            if not invMat and not invProp:
                ...
                dMdprop =  (Utils.sdiag(MI.diagonal()**2) * Av.T * V *
                            Utils.sdiag(1./prop**2))
        ...
        if dMdprop is not None:
            def innerProductDeriv(v=None):
                if v is None:
                    print ("Depreciation Warning: TensorMesh.innerProductDeriv."
                           " You should be supplying a vector. Use: "
                           "Utils.sdiag(u)*dMdprop")
                    return dMdprop
