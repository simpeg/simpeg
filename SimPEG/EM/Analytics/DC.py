...
def DCAnalyticSphere_Generalized(txloc, rxloc, sphloc, radius, sigma, sigma1,
    ...
    :param array txloc: A (+) current electrode location (x, y, z)
    :param array sphloc: location of sphere center (x, y, z)
    :param array rxloc: M(+) electrode locations / (Nx3 array, # of electrodes)
    ...
    xyz = np.c_[rxloc[:, 0] - sphloc[0], rxloc[:, 1] - sphloc[1], rxloc[:, 2] - sphloc[2]]

    # r: vec from sphere center to rxloc
    rx = rxloc[:, 0] - sphloc[0]
    ry = rxloc[:, 1] - sphloc[1]
    rz = rxloc[:, 2] - sphloc[2]
    r = np.sqrt((np.c_[rx, ry, rz]**2).sum(axis=1))

    # rs: vec from sphere center to txloc
    rsx = txloc[0] - sphloc[0]
    rsy = txloc[1] - sphloc[1]
    rsz = txloc[2] - sphloc[2]
    rs = np.sqrt(rsx**2 + rsy**2 + rsz**2)

    assert rs>radius, "Tx must lie outside of the sphere!"

    # cos(theta) = (\vec(rs) dot \vec(r))/(||\vec(rs)|| dot ||\vec(r)||)
    costheta = (rsx*rx + rsy*ry + rsz*rz)/(rs*r)

    # R: distance from txloc to rxloc
    R = np.sqrt(r**2+rs**2.-2.*r*rs*costheta)
...
def AnBnfun(n, radius, rs, rho, rho1, I=1.):
    const = I*rho/(4*np.pi)
    bunmo = n*rho + (n+1)*rho1
    An = const * radius**(2*n+1) / rs ** (n+1.) * n * \
        (rho1-rho) / bunmo
    Bn = const * 1. / rs ** (n+1.) * (2*n+1) * (rho1) / bunmo
    return An, Bn
