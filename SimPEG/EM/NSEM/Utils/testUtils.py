...
def getAppResPhs(NSEMdata):
    ...
    for src in NSEMdata.survey.srcList:
        ...
        zList.append(zc)
    return [appResPhs(zList[i][0], np.sum(zList[i][1:3]))
...
def setup1DSurvey(sigmaHalf, tD=False, structure=False):
    ...
    core = np.concatenate((
        np.kron(meshTensor([(ct, 15, -1.2)]), np.ones((10, ))),
        meshTensor([(ct, 20)])))
...
def setupSimpegNSEM_ePrimSec(
    inputSetup, tf_type='Imp', singleFreq=False, expMap=True):
    ...
    if tf_type in ['HMV', 'All']:
        for rx_type in ['xx', 'xy', 'yx', 'yy']:
            ...
            rxList.append(Point_horizontalmagvar3D(
                locs=rx_loc_mat, orientation=rx_type, component='real'))
            rxList.append(Point_horizontalmagvar3D(
                locs=rx_loc_mat, orientation=rx_type, component='imag'))
    # Source list
    srcList = []
    ...
    if expMap:
        simulation = Simulation3D_ePrimSec(
            mesh=M, survey=survey, sigmaPrimary=np.log(sigma1d))
        simulation.sigmaMap = simpeg.Maps.ExpMap(simulation.mesh)
        simulation.model = np.log(sig)
    else:
        simulation = Simulation3D_ePrimSec(
            mesh=M, survey=survey, sigmaPrimary=sigma1d)
        simulation.sigmaMap = simpeg.Maps.IdentityMap(simulation.mesh)
...
def getInputs():
    ...
    M = simpeg.Mesh.TensorMesh(
        [
            [(200, 6, -1.5), (200., 4), (200, 6, 1.5)],
            [(200, 6, -1.5), (200., 4), (200, 6, 1.5)],
            [(200, 8, -1.5), (200., 8), (200, 8, 1.5)]],
        x0=['C', 'C', 'C'])
    # Set the frequencies
    ...
    rx_x, rx_y = np.meshgrid(
        np.arange(-350, 350, 200), np.arange(-350, 350, 200))
    rx_loc = np.hstack((
...
def blockInhalfSpace(conds):
    ...
    sig = simpeg.Utils.ModelBuilder.defineBlock(
        M.gridCC,
        np.array([-1000, -1000, -1500]), np.array([1000, 1000, -1000]), conds)
    sig[~groundInd] = 1e-8
    # Set the background, not the same as the model
    sigBG = np.zeros(M.nC) + 1e-8
