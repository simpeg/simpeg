...
class Fields1D_eTotal(BaseNSEMFields):
    ...
    aliasFields = {
                    'e_1d' : ['e_1dSolution','F','_e'],
                    'e_1dBounds' : ['e_1dSolution','F','_eBounds'],
                    'e_1dInterior' : ['e_1dSolution','F','_eInterior'],
                    'b_1d' : ['e_1dSolution','E','_b'],
                    'b_1dBounds' : ['e_1dSolution','E','_bBounds'],
                    'b_1dInterior' : ['e_1dSolution','E','_bInterior']
                  }

    def __init__(self,mesh,survey,**kwargs):
        BaseNSEMFields.__init__(self,mesh,survey,**kwargs)

    def _eBounds(self, eSolution, srcList):
        """
        Bounds electric field from source
        ...
        :return: primary electric field as defined by the sources
        """
        eBounds = np.zeros_like(eSolution)
        for i, src in enumerate(srcList):
            ep = src.eBounds(self.survey.prob)
            if ep is not None:
                eBounds[:,i] = ep[:,-1]
        return eBounds

    def _eInterior(self, eSolution, srcList):
        """
        Interior electric field is the thing we solved for
    ...
    def _e(self, eSolution, srcList):
        ...
        :return: total electric field
        """
        return self._eBounds(eSolution,srcList) + self._eInterior(eSolution,srcList)
    ...
    def _bBounds(self, eSolution, srcList):
        bBounds = np.zeros([self.survey.mesh.nE,eSolution.shape[1]], dtype = complex)
        for i, src in enumerate(srcList):
            bp = src.bBounds(self.survey.prob)
            if bp is not None:
                bBounds[:,i] += bp[:,-1]
        return bBounds

    def _bInterior(self, eSolution, srcList):
        """
        Bounds magnetic flux density from source
    ...
    def _b(self, eSolution, srcList):
        ...
        :return: total magnetic field
        """
        return self._bBounds(eSolution, srcList) + self._bInterior(eSolution, srcList)
    ...
    def _bDeriv_u(self, src, v, adjoint = False):
        ...
        :return: product of the derivative of the magnetic flux density with respect to the field we solved for with a vector
        """
        # bBounds: no model depenency
        C = self.mesh.nodalGrad
        if adjoint:
            bInteriorDeriv_u = - 1./(1j*omega(src.freq)) * (C.T * v)
        else:
            bInteriorDeriv_u = - 1./(1j*omega(src.freq)) * (C * v)
        return bInteriorDeriv_u
    ...
    def _bDeriv_m(self, src, v, adjoint = False):
        ...
        :return: product of the magnetic flux density derivative with respect to the inversion model with a vector
        """
        # Neither bBounds nor bSeconary have model dependency => return Zero
        return Zero()
...
class Fields3D_ePrimSec(BaseNSEMFields):
    ...
    def _e_pyDeriv_u(self, src, v, adjoint = False):
        ...
        # e_pyPrimary doesn't depend on u, only e_pxSecondary
        if adjoint:
