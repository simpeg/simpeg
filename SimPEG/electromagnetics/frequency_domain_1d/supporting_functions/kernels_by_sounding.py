...
def magnetic_dipole_response_by_sounding(
    ...
    if output_type == "sensitivity_sigma":
        drTE, _, _ = rTE_gradient(f, lamda.reshape(-1), sig, mu, thicknesses)
        drTE.reshape((n_layer, n_frequency, n_filter))
        temp = drTE * np.exp(-lamda * (z + h))
    else:
        rTE = rTE_forward(f, lamda.reshape(-1), sig, mu, thicknesses)
    ...
    for src in source_list:
        for rx in src.receiver_list:
            ...
            if isinstance(simulation, fdem_1d.EM1DFMSimulation):
                if output_type == "sensitivity_sigma":
                    temp_slice = temp[:, i_count, :].reshape((n_layer, 1, n_filter))
                else:
            ...
            if np.all(src.orientation == [0, 0, 1]):
                if rx.orientation == "z":
                    kernels = [C * lamda[0, :] ** 2 * temp_slice, None, None]
                elif rx.orientation == "x":
                    C *= -v_dist[0] / np.sqrt(np.sum(v_dist[0:-1] ** 2))
                    kernels = [None, C * lamda[0, :] ** 2 * temp_slice, None]
                ...
                if rx.orientation == "z":
                    ...
                    kernels = [
                        C0 * lamda[0, :] ** 2 * temp_slice,
                        C1 * lamda[0, :] * temp_slice,
                        None,
                    ]
                elif rx.orientation == "y":
            ...
            data_or_sensitivity[src, rx] = simulation.project_fields_src_rx(
                integral_output, src.i_sounding, src, rx, output_type=output_type
            )
            i_count += 1
        ...
        for rx in src.receiver_list:
            ...
            if isinstance(simulation, fdem_1d.EM1DFMSimulation):
                if output_type == "sensitivity_sigma":
                    kernels = (
                        None,
                        kernel[:, icount, :].reshape(n_layer, 1, n_filter),
                        None,
                    )
                else:
                    kernels = (None, kernel[icount, :], None)
            elif isinstance(simulation, tdem_1d.EM1DTMSimulation):
