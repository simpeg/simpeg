...
This release ships a new and improved version of the magnetic differential
simulation
...
numerically solving the magnetostatic PDE. It takes into account
self-demagnetization effects, it can model induced and remanent magnetizations,
and it's faster and less memory intensive than the integral simulation on large
problems.
...
Add new `shift_to_discrete_topography` function that shifts locations relative
to discrete surface topography. We measure electric fields at the Earth's
surface when performing MT surveys. Like DC/IP, the original measurement
locations of the electric fields can end up in air cells when we define
discrete surface topography. These functions allow the user to shift locations
...
Now the gravity and magnetic equivalent sources can define the sensitivity
matrix ``J`` as a :class:`scipy.sparse.linalg.LinearOperator` when
``store_sensitivities="forward_only"``. This extends the behaviour that was
already implemented in the integral gravity and magnetic simulations to the
...
:mod:`simpeg.utils` to make it easier to use it.

Check out the new How to Guide on :ref:`choosing-solvers` for more information
on how we can use this function.

Improve inversion print out
~~~~~~~~~~~~~~~~~~~~~~~~~~~

The information table we see when running an inversion got improved. Now the
zeroth iteration corresponds to the status of the inversion problem **before**
running any optimization step, while the following ones show information about
the problem **after** each iteration, but before directives are applied. Now
the last iteration of the inversion is shown in the last row of the table.
Moreover, some non-very-useful messages have been removed to produce a cleaner
output.

See https://github.com/simpeg/simpeg/pull/1626 for more details.

Standardize directives to save outputs
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The directives that are meant to store and save inversion outputs have been
standardize and made more reliable. They now respect the output directory
chosen by the user, and the output file names have been standardize to follow
a ``name-timestamp-iteration`` format, to make it easier to sort and identify
...
The conjugate gradient minimizers were updated to work with the ones in latest
versions of SciPy. They can now take both relative and absolute tolerances
...
We also included a new page that clarifies Python and Numpy :ref:`versions
compatibility` with SimPEG, and explain the criteria for dropping older
...
Finally, we improved a fixed a few things in the docs: mathematical
...
:ref:`version-compatiblity` schedule. So, remember to use Python 3.11 or higher
...
The :func:`~simpeg.utils.model_builder.get_indices_block` function used to
return a tuple with just a single element: the array with cell indices that
correspond the given block. We standardize its output to be in agreement with
similar functions in the module. Now it returns a single Numpy array with the
cell indices that correspond to the block.
...
We also removed some bits of code that were marked for deprecation in previous
releases, like:
