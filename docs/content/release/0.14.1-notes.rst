...
``m_locations`` and ``n_locations``. There are now two ways to create a ``Dipole`` receiver:

 .. code-block:: python

  rx = resistivity.receivers.Dipole(m_locations, n_locations)

or

.. code-block:: python

  rx = resistivity.receivers.Dipole(locations=(m_locations, n_locations))
