...
from ..resistivity.simulation import Simulation3DNodal as SimulationDC3D
...
class Simulation3DNodal(BaseSimulation, Sim):
    ...
    def Jvec(self, m, v, f=None):
        ...
        if isinstance(self.Jmatrix, np.ndarray):
            return self._scale.astype(np.float32) * (
                self.Jmatrix @ v.astype(np.float32)
            )

        if isinstance(self.Jmatrix, Future):
            self.Jmatrix  # Wait to finish

        return self._scale.astype(np.float32) * da.dot(self.Jmatrix, v).astype(
            np.float32
        )
    ...
    def Jtvec(self, m, v, f=None):
        ...
        if isinstance(self.Jmatrix, np.ndarray):
            return (self._scale * v.astype(np.float32)).astype(
                np.float32
            ) @ self.Jmatrix
...
Simulation3DNodal.compute_J = SimulationDC3D.compute_J
Simulation3DNodal.getSourceTerm = SimulationDC3D.getSourceTerm
